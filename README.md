...
## API project: Strikeout Lab

My API project lives in its own repository: **[andrewjounkim/strikeout-lab](https://github.com/andrewjounkim/strikeout-lab)**
(featured on the Projects page). It is an interactive Python (Streamlit) app that models MLB pitcher strikeouts
from the free, keyless MLB Stats API: you can browse pitchers and teams, see today's games with each probable
starter modeled against the opposing lineup, and build any matchup to get an expected strikeout total and the
chance of going over or under a hypothetical line. Its README covers how the API is called, the model and its
assumptions, how to run it, and the prompt log. Because it is a Python app that runs locally, this site links to
the code and shows screenshots instead of embedding it.
...
- Built with Claude (Anthropic) as described above; Strikeout Lab's prompt log is in its own repository.
